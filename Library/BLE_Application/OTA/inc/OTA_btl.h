 ...
 */
  ...
  /** @brief MAX ATT_MTU size (< 251; max LL data packet length) allowed from both OTA Client (i.e. DTM application) and Server sides.
    * It is  used for OTA transfer with data length extension feature on BlueNRG-2, BLE stack v2.1 or later.
    */ 
  #define OTA_ATT_MTU_SIZE     (220)              //i.e. OTA Client, DTM framework, example: MAX_ATT_MTU = 220 (DTM_config.h)              
#else 
  /**  @brief MAX ATT_MTU size with no data length extension support */   
  #define OTA_ATT_MTU_SIZE     (DEFAULT_ATT_MTU)  // DEFAULT_ATT_MTU size = 23 bytes   
#endif
