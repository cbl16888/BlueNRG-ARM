...
tBleStatus Add_Acc_Service(void)
{
  ...
  Osal_MemCpy(&service_uuid.Service_UUID_128, uuid, 16); //copy de la valeur du UUID de ACC vers la variable service_uuid de type Service_UUID_t_s qui puisse être utilisé par les fonctions suivantes
  ret = aci_gatt_add_service(UUID_TYPE_128,  &service_uuid, PRIMARY_SERVICE, Services_Max_Attribute_Records[ACCELERATION_SERVICE_INDEX], &accServHandle); //creation du service avec le UUID, le type de UUID, le handle du service et le nombre de char contenu dans le service
  if (ret != BLE_STATUS_SUCCESS) goto fail;    

  COPY_ACC_UUID(uuid);  
  Osal_MemCpy(&char_uuid.Char_UUID_128, uuid, 16);
  ret =  aci_gatt_add_char(accServHandle, UUID_TYPE_128, &char_uuid, 180, CHAR_PROP_NOTIFY|CHAR_PROP_READ, ATTR_PERMISSION_NONE, GATT_DONT_NOTIFY_EVENTS,
                           16, 0, &accCharHandle);
  if (ret != BLE_STATUS_SUCCESS) goto fail;

  COPY_START_UUID(uuid);
  Osal_MemCpy(&char_uuid.Char_UUID_128, uuid, 16);
  ret =  aci_gatt_add_char(accServHandle, UUID_TYPE_128, &char_uuid, 1, CHAR_PROP_WRITE_WITHOUT_RESP | CHAR_PROP_READ, ATTR_PERMISSION_NONE, GATT_NOTIFY_ATTRIBUTE_WRITE ,
                           16, 0, &startCharHandle);
  if (ret != BLE_STATUS_SUCCESS) goto fail;

  PRINTF("Service ACC added. Handle 0x%04X, Acc Charac handle: 0x%04X, ", accServHandle,  accCharHandle);
  PRINTF("Start Charac handle: 0x%04X\n",startCharHandle);
...
{
  uint8_t buff[180]; //make room for 6*8 bits of data
  tBleStatus ret;
  static int i=0;
  PRINTF("Test %u \n", i);

  memcpy(buff, buffer+(180*sizeof(uint8_t)*i), 180*sizeof(uint8_t));

  ret = aci_gatt_update_char_value_ext(connection_handle, accServHandle, accCharHandle, 1, 180, 0, 180, buff);
  if (ret == BLE_STATUS_SUCCESS){
	  i++;
	  if(i==3){
		  i=0;
